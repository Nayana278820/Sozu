...
class Waterfall(object):
    ...
    def update(self, *args):
        ...
        for scan_num, start_ind in enumerate(range(0, NUM_SCANS_PER_SWEEP*NFFT, NFFT)):
            ...
            # Calculate the frequency range based on the current center frequency and sampling rate
            center_freq = self.sdr.fc
            freq_range = (center_freq - self.sdr.rs / 2) + np.arange(NFFT) * (self.sdr.rs / NFFT)
            # print("Frequency range:", freq_range)

            # Write frequency changes to CSV file
            with open('psdFreq_changes_newCircuit2.csv', 'a', newline='') as csvfile:
                    # one for the psd chnages, frequency changes
                writer = csv.writer(csvfile)

                # writer.writerow([])
                # writer.writerow(["PSD Values"])
                    # Collect PSD values into a list
                # psd_data = []
                # for value in psd_scan:
                #     psd_data.append([value])  
                # Collect PSD values into a list
                psd_data = [[value] for value in psd_scan]

                    # Transpose the list to have each PSD scan as a column
                psd_data_transposed = np.array(psd_data).T.tolist()
            
                # Write the transposed data to the CSV file
                writer.writerows(psd_data_transposed)

                #     # Add a header for frequencies
                # writer.writerow(["Frequencies (MHz)"])

                # # # Calculate the frequencies associated with each PSD value
                # # freq_range_start = (self.sdr.fc - self.sdr.rs / 2) / 1e6
                # # freq_step = self.sdr.rs / NFFT / 1e6
                # # frequencies = [freq_range_start + i * freq_step for i in range(NFFT)]
    
                # # Write the frequencies to the CSV file
                # # for i in freq_range:
                # writer.writerow(freq_range)
