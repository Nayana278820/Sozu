...
class Waterfall(object):
    ...
    def __init__(self, sdr=None, fig=None):
        ...
        self.init_plot()
        self.df = pd.DataFrame(columns=[str(i) for i in range(NFFT)])
    ...
    def update(self, *args):
        ...
        for scan_num, start_ind in enumerate(range(0, NUM_SCANS_PER_SWEEP*NFFT, NFFT)):
            ...
            psd_values.extend(psd_scan.tolist())
            # Append PSD values to DataFrame
            self.df = self.df.append(pd.Series(psd_values, index=self.df.columns), ignore_index=True)
